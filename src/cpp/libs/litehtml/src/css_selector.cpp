...
wq_name parse_type_selector(const css_token_vector& tokens, int& index) {
    ...
    if (tok.type == IDENT || tok.ch == '*') {
        ...
        // type selector is always ASCII-case-insensitive for HTML documents, regardless of document
...
bool parse_attr_matcher(const css_token_vector& tokens, int& index, attr_matcher& matcher) {
    ...
    if (!(is_one_of(a.ch, '~', '|', '^', '$', '*') && b.ch == '=')) return false;
...
css_attribute_selector parse_attribute_selector(const css_token& block) {
    ...
    // attribute name in attribute selector is ASCII case-insensitive for HTML documents, regardless
    // of document mode (quirks/no quirks)
    ...
    if (!parse_attr_matcher(tokens, index, matcher)) return {};

    // <string-token> | <ident-token>
    skip_whitespace(tokens, index);
    const css_token& value = at(tokens, index);
    if (value.type != STRING && value.type != IDENT) return {};
    index++;
    ...
    skip_whitespace(tokens, index);
    if (index != (int)tokens.size()) return {};  // junk at the end of attribute selector

    // https://html.spec.whatwg.org/multipage/semantics-other.html#case-sensitivity-of-selectors
    // Attribute selectors on an HTML element in an HTML document must treat the values
    // of attributes with the following names as ASCII case-insensitive (unless s modifier is
    // specified):
...
bool to_int(string s, int& number) {
    ...
    if (end != ptr + s.size()) return false;
...
css_attribute_selector parse_nth_child(const css_token& token, bool of_keyword,
    ...
    if (i >= 0) {
        ...
        // https://www.w3.org/TR/selectors-4/#structural-pseudos So I parse as if they were not
        // allowed.
        ...
        // Chrome/Firefox behavior differs from the standard: they treat S as unforgiving and allow
        // pseudo-elements. NOTE: :is(), which also accepts <forgiving-selector-list>, is handled
        // correctly by Chrome/Firefox. Use this code instead of above to match Chrome/Firefox
        // behavior:
        // selector.selector_list = parse_selector_list(selector_tokens, strict_mode);
        // if (selector.selector_list.empty()) return {};
    }

    // get string representation of everything between "nth-child(" and "of" or ")", except for
    // comments
...
css_attribute_selector parse_function_pseudo_class(const css_token& token, document_mode mode) {
    ...
    } else if (name == "is" || name == "where")  // https://www.w3.org/TR/selectors-4/#matches
    {
        ...
        selector.selector_list =
            parse_selector_list(token.value, forgiving_mode + forbid_pseudo_elements, mode, false);
        return selector;
    } else if (name == "has")  // https://www.w3.org/TR/selectors-4/#relational
    {
        css_attribute_selector selector(select_pseudo_class, name);
        selector.selector_list =
            parse_selector_list(token.value, strict_mode + forbid_pseudo_elements, mode, true);
        if (selector.selector_list.empty()) return {};
        return selector;
    } else if (name == "not")  // https://www.w3.org/TR/selectors-4/#negation
    {
        ...
        selector.selector_list =
            parse_selector_list(token.value, strict_mode + forbid_pseudo_elements, mode, false);
        if (selector.selector_list.empty()) return {};
        return selector;
...
bool is_supported_simple_pseudo_class(const string& name) {
    ...
    if (n == "checked" || n == "disabled" || n == "enabled" || n == "required" || n == "invalid" ||
        n == "valid")
        return true;
...
css_attribute_selector parse_pseudo_class(const css_token_vector& tokens, int& index,
    ...
    if (b.type == IDENT) {
        // unsupported pseudo-classes must be treated as invalid:
        // https://www.w3.org/TR/selectors-4/#w3c-partial
        if (!is_supported_simple_pseudo_class(b.ident())) return {};
...
// <pseudo-class-selector> <id-selector> = <hash-token> with hash_type == ID <class-selector> = '.'
// <ident-token>
        ...
        case '.':
            if (tok1.type == IDENT || tok1.ch == '\\') {
                ...
                while (index < (int)tokens.size()) {
                    ...
                    if (t.type == WHITESPACE && nest_level == 0) break;
...
bool is_supported_simple_pseudo_element(const string& name) {
    string n = lowcase(name);
    if (is_one_of(n, "before", "after", "marker", "placeholder", "backdrop", "file-selector-button",
                  "selection"))
        return true;
...
css_attribute_selector parse_pseudo_element(const css_token_vector& tokens, int& index) {
    ...
    if (a.ch != ':') return {};
    if (b.ch != ':' && b.type != IDENT) return {};

    if (b.type == IDENT)  // legacy syntax with one ':'
    ...
    {
        if (!is_supported_simple_pseudo_element(c.ident())) return {};
...
css_element_selector::ptr parse_compound_selector(const css_token_vector& tokens, int& index,
    ...
    while (true) {
        ...
        while ((sel = parse_pseudo_class(tokens, index, mode))) selector->m_attrs.push_back(sel);
    }

    // [..]!  "must produce at least one value"  https://www.w3.org/TR/css-values-4/#mult-req
    if (selector->m_tag == empty_id && selector->m_attrs.empty()) return nullptr;

    if (selector->m_tag == empty_id) selector->m_tag = star_id;
...
// column combinator || is at-risk https://www.w3.org/TR/selectors-4/  and not implemented in
// Chrome/Firefox  https://caniuse.com/mdn-css_selectors_column
...
css_selector::ptr parse_complex_selector(const css_token_vector& tokens, document_mode mode,
    ...
    // NOTE: all the whitespace is handled by parse_combinator, that's why skip_whitespace is never
    // called in the loop NOTE: parse_complex_selector is different from most other parse_xxx
    // functions in that it's required
    //       to parse all input tokens, it doesn't just parse as much as possible.
    while (true) {
        int combinator = parse_combinator(tokens, index);
        if (index == (int)tokens.size())
            // combinator == 0 means index already was at the end before the call to
            // parse_combinator
            return !combinator || combinator == ' ' ? selector : nullptr;
        if (!combinator)  // not the end and combinator failed to parse
            return nullptr;

        // otherwise: index is not at the end, combinator is good and tokens[index] is not
        // whitespace it means if parse_compound_selector fails it's an error
...
bool has_selector(const css_selector& selector, attr_select_type type, const string& name = "") {
    for (const auto& sel : selector.m_right.m_attrs) {
        if (sel.type == type && (name == "" || equal_i(_s(sel.name), name))) return true;
    }

    if (selector.m_left) return has_selector(*selector.m_left, type, name);
...
css_selector::vector parse_selector_list(const css_token_vector& tokens, int options,
                                         document_mode mode, bool relative) {
    ...
    for (const auto& list : list_of_lists) {
        ...
        if (!selector || ((options & forbid_pseudo_elements) &&
                          has_selector(*selector, select_pseudo_element))) {
            if (options & forgiving_mode) continue;
