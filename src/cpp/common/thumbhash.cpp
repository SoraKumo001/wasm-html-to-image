...
std::vector<uint8_t> rgbaToThumbHash(int w, int h, const uint8_t *rgba)
{
  ...
  for (int i = 0; i < size; i += 4)
  {
    ...
    for(int j=0; j<count; ++j) {
        float alpha_f_val = rgba[(i+j)*4+3] * inv_255;
        float r_f_val = rgba[(i+j)*4+0] * inv_255 * alpha_f_val + avg_r * (1.0f - alpha_f_val);
        float g_f_val = rgba[(i+j)*4+1] * inv_255 * alpha_f_val + avg_g * (1.0f - alpha_f_val);
        float b_f_val = rgba[(i+j)*4+2] * inv_255 * alpha_f_val + avg_b * (1.0f - alpha_f_val);

        l[i+j] = (r_f_val + g_f_val + b_f_val) / 3.0f;
        p[i+j] = (r_f_val + g_f_val) / 2.0f - b_f_val;
        q[i+j] = r_f_val - g_f_val;
        a[i+j] = alpha_f_val;
    }
  }
  ...
  // --- AC coefficients ---
  int ac_start = (int)hash.size();
  int ac_index = 0;
  std::vector<std::vector<float>> ac_channels;
  if (hasAlpha) {
      ac_channels = {l_ac, p_ac, q_ac, a_ac};
  } else {
      ac_channels = {l_ac, p_ac, q_ac};
  }
  ...
  for (const auto &ac : ac_channels)
  {
    for (float f : ac)
    {
      int byteOffset = (ac_index >> 1);
      int byteIndex = ac_start + byteOffset;
      int shift = (ac_index & 1) << 2;
      if (byteIndex >= (int)hash.size())
        hash.push_back(0);
